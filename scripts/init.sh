...
install_basic_packages() {
    ...
    # mlocate / plocate 兼容：优先使用 plocate（Ubuntu 24.04 默认）
    if ! command -v updatedb >/dev/null 2>&1; then
        apt-get install -y plocate mlocate || true
    fi
...
EOF_PYENV
        ...
        log_info "未检测到 EXTERNALLY-MANAGED，升级系统 pip (带 --break-system-packages)"
        $PIP_CMD install -U -i https://mirrors.aliyun.com/pypi/simple/ \
            --trusted-host mirrors.aliyun.com \
            --no-input \
            --upgrade pip wheel setuptools \
            --break-system-packages || log_warning "系统 pip 升级部分失败"
    fi
    ...
    if [ -f "$EXTERNALLY_MANAGED_FILE" ]; then
        ...
        # 非受管环境下允许带 --break-system-packages
        $PIP_CMD install -U -i https://mirrors.aliyun.com/pypi/simple/ \
            --trusted-host mirrors.aliyun.com \
            --no-input \
